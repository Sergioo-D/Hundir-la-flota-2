...
from persistencia import obtenerUsuario
...
def elJuego(idUserLogeado):
    ...
    def crear_interfaz():
        ...
        global l_jugador
        l_jugador = tk.Label(window, text=f"Turno {user[1]}: 0")
        l_jugador.grid(row=10, column=5, columnspan=2)
        botonCambiarJugador = tk.Button(window, text="Cambiar de jugador", width=15, command=volverLogin)
